...
* Enqueue theme fonts.
*/
 if(get_theme_mod('spicepress_enable_local_google_font',true) ==true){
add_action( 'wp_enqueue_scripts', 'spicepress_theme_fonts',1 );
add_action( 'enqueue_block_editor_assets', 'spicepress_theme_fonts',1 );
add_action( 'customize_preview_init', 'spicepress_theme_fonts', 1 );

function spicepress_theme_fonts() {
    $fonts_url = spicepress_get_fonts_url();
    // Load Fonts if necessary.
    if ( $fonts_url ) {
        require_once (get_theme_file_path( '/functions/font/wptt-webfont-loader.php' ));
        wp_enqueue_style( 'spicepress-theme-fonts', wptt_get_webfont_url( $fonts_url ), array(), '20201110' );
    }
}
}
/**
 ...
 */
function spicepress_get_fonts_url() {
    ...
    return apply_filters( 'spicepress_get_fonts_url', add_query_arg( $query_args, 'https://fonts.googleapis.com/css' ) );
}
?>